...
const DateRangeSelect = (props) => {
    const domainMin = moment("1850-01-01T20:00:00+0800").valueOf();
    const domainMax = moment().valueOf();

    const [selected, setSelected] = React.useState([domainMin, domainMax]);
    ...
    function formatTime(ms) {
        let format = moment(ms).format('L');
        return format;
    }

    function formatTickTime(ms) {
        let format = moment(ms).format('YYYY');
        return format;
    }
    ...
    return (
        <>
            <Slider
                ...
                domain={[+min, +max]}
                values={[+min, +max]} // Set handles number & initial position
                step={1}
                mode={3}
                onChange={onChange}
            >
