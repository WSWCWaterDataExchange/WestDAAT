...
function useWaterRightDigestMapPopup() {
  ...
  const result = useMemo(() => {
    ...
    if (isFetching) {
      return <LoadingCard onClosePopup={handleClosePopup} loadingText={`Retrieving water right data for ${siteUuid}`} />;
    }
    if (!siteData || siteData.length === 0) {
      return <ErrorCard onClosePopup={handleClosePopup} errorText={`Unable to find water right data for ${siteUuid}`} />
    }
    return <WaterRightsDigestCard siteUuid={siteUuid} waterRights={siteData} onSelectedIndexChanged={setCurrentIndex} currentIndex={currentIndex} onClosePopup={handleClosePopup} />;
