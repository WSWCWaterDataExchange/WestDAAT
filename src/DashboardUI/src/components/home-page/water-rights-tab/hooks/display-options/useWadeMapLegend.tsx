...
export function useWadeLegend() {
    ...
    const renderedMapGroupingsColors = useMemo(() => {
      ...
      if (mapGroupingOption === MapGrouping.BeneficialUse as string && beneficialUseFilters && beneficialUseFilters?.length > 0) {
    ...
    const legendItems = useMemo(() => {
      ...
      return renderedMapGroupingsColors.map(layer => {
        return <MapLegendCircleItem key={layer.key} color={layer.color}>{layer.key}</MapLegendCircleItem>
      })
    ...
    }, [mapGroupingOption, fallbackColor, setLayerCircleColors, setLayerFillColors, mapGroupingColors, renderedMapGroupingsColors])
    return {legendItems};
}