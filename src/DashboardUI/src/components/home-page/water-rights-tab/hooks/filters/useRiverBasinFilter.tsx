...
export function useRiverBasinFilter() {
  ...
  const { filters: { riverBasinNames: rbn }, setFilters } = useContext(WaterRightsContext);
  ...
  const setRiverBasinNames = useCallback((riverBasinNames: string[] | undefined) => {
    setFilters(s => ({
      ...s,
      riverBasinNames: riverBasinNames === undefined || riverBasinNames.length === 0 ? undefined : [...new Set(riverBasinNames)]
    }));
  }, [setFilters]);

  return { riverBasinNames, mapFilters, riverBasinPolygonsQuery, setRiverBasinNames };
}
