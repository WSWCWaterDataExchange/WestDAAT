...
export function useNldiFilter() {
  ...
  const nldiWadeSiteIds = useMemo(() => {
    ...
    let arr = nldiGeoJsonData.features
  ...
  const setNldiMapActiveStatus = useCallback((isActive: boolean) => {
    setFilters(s => ({
      ...s,
  ...
  const setLatLong = useCallback((latitude: number | null, longitude: number | null) => {
    setFilters(s => ({
      ...s,
      nldiFilterData: {
  ...
  const setDataPoints = useCallback((dataPoints: DataPoints) => {
    setFilters(s => ({
      ...s,
      nldiFilterData: {
        ...(s.nldiFilterData ?? defaultNldiFilters),
  ...
  const setDirections = useCallback((directions: Directions) => {
    setFilters(s => ({
      ...s,
      nldiFilterData: {
        ...(s.nldiFilterData ?? defaultNldiFilters),
  ...
  return { isNldiFilterActive, nldiFilterData, nldiFeaturesQuery, setNldiMapActiveStatus, setLatLong, setDataPoints, setDirections, mapFilters };
}
