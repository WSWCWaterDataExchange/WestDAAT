...
beforeEach(() => {
  jest.spyOn(WaterRightsProvider, 'useWaterRightsContext').mockImplementation(() => ({
  ...
  jest.spyOn(NldiQueries, 'useNldiFeatures').mockImplementation(() => ({ data: undefined } as any));
});
...
describe('setLatLong', () => {
  test.each([
    ...
    [
      ...
      20,
      -80,
      { ...WaterRightsProvider.defaultNldiFilters, latitude: 20, longitude: -80 },
    ],
