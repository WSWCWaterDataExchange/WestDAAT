...
export function useMapLegend() {
    ...
    const sites = useMemo(() => {
      ...
      return siteLocations?.features.map(buildMapLegend).filter(a=>!!a) as {uuid: string, podOrPou?: string}[] || [];
    }, [siteLocations]);
    ...
    useEffect(() => {
      ...
      sites.forEach((a, i) =>{
        ...
        legendItems.push(<MapLegendMarkerItem key={a.uuid} color={color}>{buildLegendString(a.uuid, a.podOrPou)}</MapLegendMarkerItem>);
      ...
      setLayerFillColors({layer: defaultPolygonFillColors.layer, fillColor: fillColor});
      setLayerIconImages({layer: defaultMapMarkerIcons.layer, iconImages: markerIcons});
