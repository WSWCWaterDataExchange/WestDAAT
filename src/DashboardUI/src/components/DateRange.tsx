...
export function DateRange(props: DateRangeProps) {
  ...
  return (
    <InputGroup className="mb-3">
      <Form.Control {...minControlProps} type="date" value={minValueString} max={moment().format("YYYY-MM-DD")} onChange={handleMinChange} />
      <InputGroup.Text className="px-2">to</InputGroup.Text>
      <Form.Control {...maxControlProps} type="date" value={maxValueString} max={moment().format("YYYY-MM-DD")} onChange={handleMaxChange} />
    </InputGroup>
  );
}
