...
function FeedbackModal(props: FeedBackModalProps) {
  ...
  const submit = async () => {
    ...
    props.setShow(false);
    const result = await postFeedback({...feedbackRequest, url: window.location.href});
    if (result === false){
        ...
        <Modal.Body>
          ...
          <div className='row mb-3'>
            <div className="col">
              ...
              <input className="form-control" placeholder="John" onChange={(e) => setFeedbackRequest({...feedbackRequest, firstName: e.target.value ?? ""})} value={feedbackRequest.firstName} />
            </div>
            <div className="col">
              <label className="fw-bolder">Last Name</label>
              <input className="form-control" placeholder="Doe" onChange={(e) => setFeedbackRequest({...feedbackRequest, lastName: e.target.value ?? ""})} value={feedbackRequest.lastName} />
            </div>
          </div>
          <div className='row mb-3'>
            <div className="col">
              <label className="fw-bolder">Email</label>
              <input type="email" className="form-control" placeholder="email@domain.com" onChange={(e) => setFeedbackRequest({...feedbackRequest, email: e.target.value ?? ""})} value={feedbackRequest.email} />
          ...
          <div className='row mb-3'>
            <div className="col">
              ...
              <input className="form-control" onChange={(e) => setFeedbackRequest({...feedbackRequest, organization: e.target.value ?? ""})} value={feedbackRequest.organization} />
            </div>
            <div className="col">
              <label className="fw-bolder">Role</label> <br />
              <input className="form-control" onChange={(e) => setFeedbackRequest({...feedbackRequest, role: e.target.value ?? ""})} value={feedbackRequest.role} />
            </div>
          </div>
          <div className='row mb-3'>
            <div className="col">
              <label className="fw-bolder">Comments</label>
              <textarea className="form-control" onChange={(e) => setFeedbackRequest({...feedbackRequest, comments: e.target.value ?? ""})} value={feedbackRequest.comments} />
            </div>
          </div>
          ...
          <div className="mb-3">
            ...
            <div className='row'>
              {dataUsageOptions.map(element =>{
                ...
                return <div className='col-4' key={idValue}>
                <div className="form-check">
                  <input
                    ...
                    onChange={e => setFeedbackRequest({...feedbackRequest, dataUsage: handleDataCheckChange(e)})}
                  />
              ...
              <div className="col-8">
                <div className='input-group input-group-sm'>
                  <label className="input-group-text" id="other-usage-label" htmlFor="data-usage-other">Other</label>
                  <input type="text" id="data-usage-other" className="form-control form-control-sm" onChange={e => setFeedbackRequest({...feedbackRequest, dataUsage: handleOtherDataChange(e)})} value={otherDataText} aria-label="Other Usage" aria-describedby="other-usage-label" />
                </div>
              </div>
            </div>
          ...
          <div className="mb-3" id="dashboardSatisfactionLevel">
            ...
            <ButtonGroup className="w-100">
              {dashboardSatisfactionOptions.map((element) => (<ToggleButton
                ...
                onChange={() => setFeedbackRequest({...feedbackRequest, satisfactionLevel: element ?? ""})}
              >
                {element}
              </ToggleButton>
              ))}
